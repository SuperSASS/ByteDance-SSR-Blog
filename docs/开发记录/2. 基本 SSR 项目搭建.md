...
4. `ssr.ts`接收到返回的`appHtml`，则与模板拼装，然后返回给浏览器。  
   ...
   可见`root`中并不为空
5. 接下来根据`<script type="module" src="http://localhost:5173/src/entry-client.tsx"></script>`，加载`entry-client.tsx`脚本（从前端的 build / vite dev 获取），进行水合，在其中调用`hydrateRoot`，从而挂上事件等东西。  
...
其中关系配置了一些级联删除，比如删除分类会删除该分类下所有文章。
...
然后通过迁移命令，其生成数据模型对应的 sql 并执行，生成 db 文件：
...
执行以下命令，生成 Prisma Client：

```bash
npx prisma generate
```

此时会按照`prisma/schema.prisma`设定的位置，生成客户端，  
其中通过`client.js`实现了对各个数据模型的 CRUD 操作。

---

上方的`client.js`只是一个类，需要实例化，  
...
其中最重要的是 Service，使用 Prisma Client 实现 CRUD，其他都是模板代码。  
下面对 Prisima Client 进行简要介绍：
