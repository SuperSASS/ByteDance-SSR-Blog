...
- **前端**
  - **框架**：React 19  
    相比 React 18 是增强升级，其简化了表单提交逻辑（Actions）、并且进一步增强了 SSR 能力。  
    本项目将使用 React 19 的`use()`、`useActionState()` 进行数据获取和行为的状态感知。
  - **语言**：TypeScript
  - **UI 库**：shadcn/ui + Tailwind CSS
  - **路由**（前端 SPA 应用 + 后端 SSR App 组件）：React Router 7  
    相比 v6 / v6.4+，其也是非破坏性升级，可直接使用。  
    更加以**数据模式(Data Mode)**为核心，即`loader`、`action`、`defer`等，路由即进行数据请求、提交处理。本项目就将采用这种模式。
    其对 SSR 更友好、可以在服务端统一执行`loader`、`action`。  
    其还增加了框架模式（Framework Mode），引入了 Remix 的框架能力
...
- 全栈框架 - Next.js / Nuxt.js(Vue 版本) / Remix  
  这三者都是“全栈开发框架”、“Fullstack React SSR Framework”，  
  可以理解为前后端合并在一起的开发框架（全栈 Web 框架），其基于 React 同时完成页面搭建和 API 开发。  
  而 Remix 是 React Router 团队推出的，是以 Web 标准为核心、以 React Router 7 为基础的全栈 SSR/SPA 框架。  
  但本项目需要从底层彻底吃透 SSR 原理，故先用 React + Koa 这种架构来实现。
- 后端应用级/企业级框架 - Nest.js  
