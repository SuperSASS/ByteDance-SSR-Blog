...
* 前后端路由结构成型：
  ...
  * `/api/*` → JSON API（先空）
  * 其它 → SSR
* React Router 前后端同步：服务端 `createMemoryRouter`，客户端 `createBrowserRouter`

### 关键技术

* React 18 SSR：`renderToString`
* React Router：`createMemoryRouter` / `createBrowserRouter` / `<RouterProvider>`
* Vite 构建 server bundle / client bundle 的思路（先用简化配置）
...
* `AppShell.tsx`：包裹 Router 的壳（后续加 QueryClientProvider / ThemeProvider）
...
<div class="background" id="background" style="background-image: url(&quot;/img/background_01.jpg&quot;);">
	<script>
